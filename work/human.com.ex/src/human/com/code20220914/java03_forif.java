...
//		}
		...
		for (int b = 0; b <= 27; b++) {
			for (int a = 0; a <= 32; a++) {
				...
				if ((a + b) != 0) {
					if ((a + b) % 3 == 0) {
						// A && B : A조건이 성립되고, B조건도 성립되어야 한다.
						// A조건 : (a+b)%3==0 -> a+b의 값을 3으로 나누어서 나머지가 0일경우(3의배수)
						// B조건 : (a+b)!=0 -> a+b의 값이 0이 아닐 경우
				...
				if (num70 == 20) {
					...
					// (num10 ==15) : num10 변수의 값이 15과 같으면 조건성립 (true)
					// *if문은 조건이 성랍되는 {} 영역에 들어가는 경우 하단 부분 조건은 생략
				} else if (num70 == 15) {
					System.out.println("num10은 15이 이다!");
				} else if (num70 == 10) {
					System.out.println("num10은 10이 이다!");
					// if문의 조건이 성립되자 않을 경우에 else{}코드영역이 실행!
